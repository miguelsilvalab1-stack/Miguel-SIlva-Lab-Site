...
export async function sendInternalLeadAlert(lead: LeadAlert) {
  ...
  try {
    ...
    const linha = (k: string, v: string) =>
      v ? `<tr><td style="padding:6px 12px;color:#666;font-size:13px;">${k}</td><td style="padding:6px 12px;color:#002D62;font-size:13px;font-weight:600;">${v}</td></tr>` : ''
    await resend.emails.send({
      ...
      html:
        ...
        '<h2 style="color:#002D62;">Novo diagnóstico concluído</h2>' +
        ...
        `<p style="margin-top:20px;"><a href="${appUrl}/diagnostico/resultado/${lead.job_id}" style="background:#002D62;color:#FDB913;padding:12px 24px;border-radius:6px;text-decoration:none;font-weight:bold;">Ver diagnóstico completo →</a></p>` +
